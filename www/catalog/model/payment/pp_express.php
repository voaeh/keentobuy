...
class ModelPaymentPPExpress extends Model {
	...
	public function paymentRequestInfo() {
		...
		$data['PAYMENTREQUEST_0_CURRENCYCODE'] = 'THB';
		...
		foreach ($this->cart->getProducts() as $item) {
			...
			$item_price = $this->currency->format($item['price'], 'THB', false, false);
			}
			else
			{
		...
		if (!empty($this->session->data['vouchers'])) {
			foreach ($this->session->data['vouchers'] as $voucher) {
				...
				if ($currencynotsupported){
				$data['L_PAYMENTREQUEST_0_AMT' . $i] = $this->currency->format($voucher['amount'], 'THB', false, false);
				}
				else
				{
		...
		foreach ($total_data as $total_row) {
			if (!in_array($total_row['code'], array('total', 'sub_total'))) {
				if ($total_row['value'] != 0) {
					if ($currencynotsupported){
					$item_price = $this->currency->format($total_row['value'], 'THB', false, false);
					}
					else
					{
					...
					if ($currencynotsupported){
					$data['L_PAYMENTREQUEST_0_AMT' . $i] = $this->currency->format($total_row['value'], 'THB', false, false);
					}
					else
					{
		...
		if ($recurring_products) {
			...
			foreach ($recurring_products as $item) {
				...
				if ($item['recurring']['trial']) {
					if ($currencynotsupported){
					$trial_amt = $this->currency->format($this->tax->calculate($item['recurring']['trial_price'], $item['tax_class_id'], $this->config->get('config_tax')), 'THB', false, false) * $item['quantity'] . ' ' . 'THB';
					}
					else
					{
				...
				if ($currencynotsupported){
				$recurring_amt = $this->currency->format($this->tax->calculate($item['recurring']['price'], $item['tax_class_id'], $this->config->get('config_tax')), 'THB', false, false)  * $item['quantity'] . ' ' . $this->currency->getCode();
				}
				else
				{
